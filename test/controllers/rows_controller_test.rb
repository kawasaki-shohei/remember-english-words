...
class RowsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create row" do
    assert_difference('Row.count') do
      post rows_url, params: { row: { jpn: @row.jpn, page: @row.page, sentense: @row.sentense, word: @row.word } }
    end
  ...
  test "should update row" do
    patch row_url(@row), params: { row: { jpn: @row.jpn, page: @row.page, sentense: @row.sentense, word: @row.word } }
